...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    if netG == 'resnet_9blocks':
        ...
        net = UNet_Nested(input_nc, output_nc, 1, is_deconv=True, is_batchnorm=False, is_ds=True)

        # net = UNet_Nested_ori(input_nc, output_nc, 1, is_deconv=True, is_batchnorm=True, is_ds=True)
...
class unetConv2(nn.Module):
    def __init__(self, in_size, out_size, is_batchnorm, n=2, ks=3, stride=1, padding=1, is_cat=False):
...
class unetUp(nn.Module):
    def __init__(self, in_size, out_size, is_deconv, n_concat=2, inner=False):
        super(unetUp, self).__init__()
        if inner==True:
            n_concat = 3
        self.conv = unetConv2(in_size + (n_concat - 2) * out_size, out_size, False, is_cat=True)
        if is_deconv:
...
class UNet_Nested(nn.Module):
    ...
    def __init__(self, in_channels=1, n_classes=2, feature_scale=1, is_deconv=True, is_batchnorm=True, is_ds=True):
        ...
        self.up_concat31 = unetUp(filters[4], filters[3], self.is_deconv, inner=True)
    ...
    def forward(self, inputs):
        ...
        return result, attention_1, attention_2, attention_12, attention_3, attention_123, attention_4

class UNet_Nested_ori(nn.Module):
    # unet++

    def __init__(self, in_channels=3, n_classes=3, feature_scale=1, is_deconv=True, is_batchnorm=True, is_ds=True):

        super(UNet_Nested_ori, self).__init__()
        self.in_channels = in_channels
        self.feature_scale = feature_scale
        self.is_deconv = is_deconv
        self.is_batchnorm = is_batchnorm
        ...
        self.final_1 = nn.Sequential(
            nn.ConvTranspose2d(filters[0], int(filters[0]/2), kernel_size=4, stride=2, padding=1),
            nn.ReLU(True),
            nn.Conv2d(int(filters[0] / 2), n_classes_1, 3, padding=1),
            nn.Tanh()
        )
        self.final_2 = nn.Sequential(
            nn.ConvTranspose2d(filters[0], int(filters[0] / 2), kernel_size=4, stride=2, padding=1),
